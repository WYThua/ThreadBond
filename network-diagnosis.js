...
try {
  const healthCheck = execSync('powershell -Command "Invoke-WebRequest -Uri \'http://localhost:3000/health\' -Method GET | Select-Object StatusCode, Content"', 
    { encoding: 'utf8' });
...
const testCases = [
  {
    name: '检查邮箱接口',
    command: 'powershell -Command "$body = @{email=\'test@example.com\'} | ConvertTo-Json; Invoke-WebRequest -Uri \'http://localhost:3000/api/auth/check-email\' -Method POST -ContentType \'application/json\' -Body $body | Select-Object StatusCode, Content"'
  },
  {
    name: '注册接口',
    command: 'powershell -Command "$body = @{email=\'test\' + (Get-Date -Format \'yyyyMMddHHmmss\') + \'@example.com\'; password=\'Test123!@#\'} | ConvertTo-Json; Invoke-WebRequest -Uri \'http://localhost:3000/api/auth/register\' -Method POST -ContentType \'application/json\' -Body $body | Select-Object StatusCode, Content"'
  }
];
